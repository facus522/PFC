...
Mediante el diagrama de casos de uso que se presenta a continuación (Figura \ref{fig:cu1} \& Figura \ref{fig:cu2}), se describe en forma gráfica la secuencia de operaciones que son llevadas a cabo por el sistema en respuesta a los eventos ejecutados por los distintos actores. Esta técnica fue utilizada para la recolección de requerimientos funcionales de la aplicación.
...
Para llevar adelante el desarrollo de la aplicación móvil se escogió el lenguaje de programación Java. Es muy conocido en el ambiente de programación Android, por ser un lenguaje que cuenta con una comunidad de más de 9 millones de desarrolladores en todo el mundo con gran experiencia en el desarrollo de software \cite{javacomunidad}. Java juega un papel muy importante, principalmente en el soporte ante casi cualquier eventualidad presentada.
\\

En los últimos años apareció el lenguaje de programación Kotlin, el cual fue adoptado por Google como lenguaje oficial de desarrollo Android. Para los fines prácticos del proyecto, los resultados podrían ser alcanzados utilizando tanto Java como Kotlin. Sin embargo, este último al poseer pocos años dentro del mercado, la cantidad de información disponible en la web es muy escasa, haciendo que a la hora de escoger entre los mencionados la decisión sea Java.
\\
...
\begin{figure}[h!]
	\begin{center}
		...
		\small\textsuperscript{\textit{Imagen extraída del libro ``Fundamentos de Programación en Java'' J.Martinez.}}
		\label{fig:jvm}
	\end{center}
\end{figure}
...
Se marca una diferencia por sobre los lenguajes interpretados, ya que estos últimos ejecutan paso a paso el código fuente sin llevar a cabo ninguna traducción en la pre-ejecución. Esto hace que se genere luego una sobrecarga en el proceso de traducción reduciendo la velocidad en tiempo de ejecución.
...
Como ya se ha especificado anteriormente Java se trata de un lenguaje de programación que soporta este paradigma. En unidades posteriores se verá con mayor detalle como el problema principal fue abstraído, adaptándose al modelo de objetos y llevando adelante dicho mecanismo.
...
Tal como se especificó anteriormente, desde la aplicación será necesaria la conexión a Internet para lograr el acceso a la base de datos. La propuesta de solución corresponde a una arquitectura cliente-servidor de 3 capas, la cuál se verá con más detalles en capítulos posteriores.
\\

La metodología de trabajo que posee esta arquitectura, es un cliente realizando peticiones (mediante el dispositivo móvil) hacia un servidor (Web Service) que se encargará de enviar las respuestas (Figura \ref{fig:cliente-servidor}). Dichas peticiones serán bajo el protocolo \textit{HTTP} y el formato de respuestas \textit{JSON}, los cuales serán ampliados en el presente capítulo.
...
\begin{figure}[h!]
	\begin{center}
		...
		\small\textsuperscript{\textit{Imagen: https://academiaandroid.com/aplicaciones-cliente-servidor-y-redes-de-telefonia-movil/}}
...
Como se mencionó anteriormente, el servidor se encontrará desplegado en el Web Service. Dicho Web Service es un conjunto de protocolos y estándares que sirven para intercambiar datos entre aplicaciones. Estas aplicaciones pueden estar desarrolladas en diferentes lenguajes de programación y siendo ejecutadas bajo cualquier plataforma de manera indiferente \cite{webServiceLibro}.
\\
...
\begin{itemize}
	...
	\item \textbf{\textit{SOAP}} (Simple Object Access Protocol): este protocolo estándar define como dos objetos en diferentes procesos pueden comunicarse por medio de intercambios de datos XML (Extensible Markup Language). Los servicios SOAP funcionan por lo general bajo el protocolo HTTP (más común), aunque no está limitado solamente a este, pudiendo trabajar con protocolos FTP, POP3, TCP, entre otros. SOAP es una arquitectura muy robusta y con gran seguridad, pero a su vez bastante pesada, tanto en tamaño como en procesamiento.
	
	\item \textbf{\textit{REST}} (Representational State Transfer): esta arquitectura es más nueva en relación a la anterior. Se puede mencionar que la misma es mucho más flexible y transporta datos mediante el protocolo HTTP, pudiendo utilizar los distintos métodos que proporciona el mismo  para la comunicación como el GET, POST, PUT, etc. Gracias a su flexibilidad permite transportar cualquier tipo de datos, como pueden ser XML, JSON (JavaScript Object Notation), binarios, etc.
...
Como se mencionó, SOAP solamente permite el formato XML lo cual lo hace más pesado en relación a REST, donde en este último se pueden enviar los datos mediante JSON (JavaScript Object Notation) por ejemplo. Brindando de esta forma una mayor flexibilidad y mejor performance, ya que estos tipos de datos son mucho más livianos en peso y más rápidos para su procesamiento por no requerir de etiquetas como en XML, además de poseer mayor soporte y ser de más fácil lectura.
\\
...
Los servicios SOAP dependen de una definición WSDL (Web Services Description Language) que describe la interfaz pública a los servicios web. En cambio, los Web Service REST proveen una sintaxis universal de modo tal que cada recurso que posee se pueda direccionar de forma unívoca a través de la URI (Identificador de recursos uniforme). El mismo es una combinación entre la URL y la URN (Uniform Resource Name), que identifica el nombre del recurso en la red en cuestión.
...
\begin{itemize}
	...
	\item La primera línea (en este caso varía para una petición o una respuesta).
...
\begin{figure}[h!]
	\begin{center}
		...
		\small\textsuperscript{\textit{Imagen: https://blog.makeitreal.camp/el-protocolo-http/}}
...
Como se mencionó anteriormente cuando el cliente realiza una petición hacia el servidor, este se encarga de enviar una respuesta al mismo. Hay servicios en los cuáles simplemente se puede responder un código de estado, otorgándole de esta manera información suficiente al cliente sobre la transacción realizada. Por ejemplo, confirmando el éxito de un ABM realizado. 
\\

A su vez, existe gran cantidad de situaciones en las cuáles es necesario enviar más información en la respuesta. Por lo tanto, es necesario determinar que formato tendrá dicha respuesta, logrando que esta sea homogénea de forma que el cliente sepa anticipadamente como decodificar la misma.
\\
...
En cuanto a la sintaxis de los JSON, los mismos están constituidos por una colección de pares nombre/valor. Los tipos de datos disponibles en estos valores son los siguientes:
...
Al realizar una petición HTTP, el contenedor de Servlets delega la misma a un Servlet en particular entre los que contiene. Este se encarga de generar la respuesta para luego entregarla al contenedor, donde este finalmente devuelve la misma al cliente en el formato adecuado (Figura \ref{fig:servlet}).
...
\begin{figure}[h!]
	\begin{center}
		...
		\small\textsuperscript{\textit{Imagen: http://tutorials.jenkov.com/java-servlets/overview.html}}
		\label{fig:servlet}
	\end{center}
\end{figure}
...
%-------------------------------------------------------------------
\label{cap2:subsec:framework}

Un \textit{framework} es un conjunto estandarizado de conceptos, prácticas y criterios para enfocar un tipo de problemática particular que sirve como referencias para enfrentar y resolver nuevos problemas de índole similar.
\\

Tienen como objetivo principal ofrecer una funcionalidad definida, auto contenida, siendo construidos usando patrones de diseño, y su característica principal es su alta cohesión y bajo acoplamiento. Sirve de base para la organización y desarrollo de software. Típicamente, puede incluir soporte de programas, bibliotecas, y un lenguaje interpretado, entre otras herramientas, para así ayudar a desarrollar y unir los diferentes componentes de un proyecto \cite{framework}.
\\

Representa una arquitectura de software que modela las relaciones generales de las entidades del dominio, y provee una estructura y una especial metodología de trabajo, la cual extiende o utiliza las aplicaciones del dominio.
\\

Dentro del proyecto para el desarrollo del Web Service, se precisará de los frameworks \textit{Spring} y \textit{Hibernate}. A grandes rasgos, mediante el uso de los mismos se facilita la inversión de control e inyección de dependencias; y el mapeo objeto-relacional entre la base de datos y el modelo de objeto, respectivamente. 
\\

A su vez, se utilizará el framework \textit{JUnit}, que tiene como funcionalidad principal la realización de test unitarios sobre el el código Java del Web Service. En capítulos posteriores se analizarán más en detalle cada uno de los mencionados.
 

\section{Conceptos del lado del cliente}
%-------------------------------------------------------------------
\label{cap2:sec:conceptos-cliente}

Una vez definidos todos los aspectos relacionados al servidor, correspondiente a la arquitectura cliente-servidor, en la presente unidad se llevará a cabo una introducción sobre conceptos relacionados al sistema operativo Android (plataforma en la que se realizó el desarrollo y despliegue de la aplicación), posicionándonos del lado cliente.

%------------------------------------------------------------------
\subsection{Sistema operativo Android}
%-------------------------------------------------------------------
\label{cap2:subsec:sistema-android}

Android es un sistema operativo diseñado fundamentalmente para trabajar dentro de dispositivos móviles, como smartphones y tablets. A lo largo de los años, se ha ido incorporando el sistema operativo en distintas tecnologías como pueden ser relojes inteligentes (wear), televisores, autos y recientemente aplicable en productos de Internet de las cosas (Android things).
\\

Inicialmente fue desarrollado por la empresa \textit{Android Inc.}, posteriormente comprada por Google en el año 2005. Una de las particularidades más interesantes que posee Android, es que se basa en Linux, plataforma libre que a su vez se basa en Unix. Esto hace que su uso sea totalmente gratuito tanto para llevar a cabo nuevos desarrollos, como para su utilización en dispositivos móviles.
\\

Cuenta con un entorno de desarrollo integrado (IDE) oficial totalmente Open Source, denominado Android Studio. El mismo fue lanzado en el año 2013, reemplazando a Eclipse como IDE oficial para el desarrollo de aplicaciones Android. Cuenta con gran cantidad de documentación en la web, así como también manuales y comunidades activas que facilitan el desarrollo y la utilización de todas las funcionalidades ofrecidas. Trae incluido el SDK (Software Development Kit) de Android, pudiendo obtener constantemente las actualizaciones propuestas por el sistema operativo. Posee como ventaja una compilación rápida, ejecución de la aplicación en tiempo real, ya sea a través del móvil y de emuladores propios, renderizado en tiempo real, etc.
\\

Hasta hace muy poco tiempo, todas las aplicaciones Android eran desarrolladas bajo el lenguaje de programación Java, ya que se trataba del lenguaje oficial para esta plataforma. En el año 2017, Google adoptó a Kotlin como lenguaje oficial de desarrollo para la plataforma Android.
\\

En el año 2008 fue lanzada la primer versión de Android denominada \textit{Apple Pie}. Con el transcurso de los años, se han ido desarrollando gran cantidad de actualizaciones, las cuáles típicamente corrigen fallos e introducen nuevas funcionalidades. En la actualidad se encuentra disponible la versión 9.0 \textit{Pie} (decimosexta versión del sistema operativo).
\\

Para el desarrollo del proyecto se utilizará como mínima versión la 4.4 \textit{(KitKat)}. La elección de la misma se debe a que las funcionalidades que presenta se encuentran acorde a los alcances del proyecto, además según las estadísticas brindadas por Android Studio (Figura \ref{fig:android_version}), aproximadamente el 95\% de los usuarios activos posee versiones superiores o iguales a la mencionada, logrando un mayor alcance de los usuarios a utilizar la aplicación.

\begin{figure}[h!]
	\begin{center}
		\includegraphics[width=0.8\textwidth]%
		{Imagenes/Bitmap/android_version}
		\caption{Versiones de Android. Estadística presentada por Android Studio siempre que se crea un nuevo proyecto desde el IDE}
		\small\textsuperscript{\textit{Imagen: https://developer.android.com/studio}}
		\label{fig:android_version}
	\end{center}
\end{figure}
...
%-------------------------------------------------------------------
\label{cap2:subsec:apis}

Una interfaz de programación de aplicaciones (API), es un conjunto de subrutinas, funciones y procedimientos que tienen la finalidad de ser utilizadas por otro software como una capa de abstracción, debido a que nos permite implementar las funciones y procedimientos que engloba en nuestro proyecto sin la necesidad de programarlas de nuevo.
\\

Representa la capacidad de comunicación entre componentes de software. Se trata del conjunto de llamadas a ciertas bibliotecas que ofrecen acceso a ciertos servicios desde los procesos y representa un método para conseguir abstracción en la programación. Uno de los principales propósitos de una API consiste en proporcionar un conjunto de funciones de uso general, donde los programadores se benefician de las ventajas del API haciendo uso de su funcionalidad, evitándose el trabajo de programar todo desde el principio.
\\

Para el desarrollo de la aplicación móvil en el presente proyecto, se utilizaron distintas APIs que facilitaron la resolución de distintas cuestiones, brindando la posibilidad de no tener que trabajar manualmente sobre cada una de ellas y de tal forma lograr un mejor producto en mejor tiempo de desarrollo.
\\

A continuación se enumeran las mismas junto a una breve descripción, teniendo en cuenta que en capítulos posteriores la información será ampliada:

\begin{itemize}
	
	\item \textit{Firebase}: plataforma utilizada para el envío de notificaciones a dispositivos Android.
	
	\item \textit{Google Services y Maps}: facilita la geolocalización de los usuarios mediante el uso de los sensores del dispositivo móvil, selección de localización manual mediante mapas (\textit{PlacePicker}) y permite la creación de mapas interactivos con la colocación de marcadores para la visualización de estadísticas.
	
	\item \textit{MPAndroidChart}: librería que brinda la posibilidad de crear gráficos para la visualización de estadísticas en los resultados de encuestas.
	
	\item \textit{OpenCsv}: brinda la posibilidad de mapear los resultados de base de datos en un archivo del tipo .csv, pudiendo exportar los datos.
	
\end{itemize}

%------------------------------------------------------------------
\subsection{Sensores de un smartphone}
%-------------------------------------------------------------------
\label{cap2:subsec:sensores-smart}

A lo largo de los años los teléfonos móviles han ido evolucionando gradualmente, siendo una de las industrias de mayor crecimiento en los últimos tiempos. Con la aparición de los smartphones (teléfonos inteligentes), los dispositivos han dejado de ser simples teléfonos móviles para convertirse en computadoras de bolsillo \cite{sensoresSmart}.
\\

Ofrecen múltiples funcionalidades y características, que permiten interactuar con el entorno y producir respuestas para el usuario, como detectar nuestra ubicación, activar el brillo automático del teléfono, etc. Las aplicaciones móviles aprovechan el contexto en el que se ejecutan para acceder a todas las capacidades en hardware y sensores que presentan los dispositivos, siendo de gran utilidad para los usuarios finales.
\\

Dentro de los sensores más comunes que se pueden encontrar en los smartphones, se presentan los siguientes \cite{sensoresSmart2}:

\begin{itemize}
	
	\item Acelerómetro: detecta el movimiento en los tres ejes diferentes, con la finalidad de obtener la orientación en la que se encuentra el dispositivo.
	
	\item Barómetro: mide la presión ambiental del aire, pudiendo obtener la altura a la que nos encontramos.
	
	\item Giroscopio: mide la aceleración no gravitacional, obteniendo dirección y movimiento angular. Sirve como complemento al acelerómetro.
	
	\item GPS: se encarga de leer continuamente la señal que le transmiten los satélites de GPS, utilizando por lo general la señal de varios de ellos para triangular la posición en la que nos encontramos.
	
	\item Lector de huella dactilar: sensor capaz de reconocer las líneas de nuestra huella dactilar y almacenar una imagen digital de la misma.
	
	\item Lector de Iris: similar al sensor de huellas, pero en este caso es capaz de reconocer los patrones de nuestro iris.
	
	\item Podómetro: mide el número de pasos que realizamos cuando estamos caminando.
	
	\item Magnetómegro: mide y cuantifica fuerzas magnéticas, sirviendo como brújula electrónica.
	
	\item Sensor de proximidad: es una combinación de un sensor infrarrojo y un sensor de luz, para por ejemplo detectar cuando tenemos el dispositivo cercano al oído y de tal forma apagar la pantalla.
	
	\item Sensor de luz ambiental: detecta la cantidad de luz que se encuentra en el ambiente, pudiendo gestionar el brillo automático de la pantalla por ejemplo.
	
	\item Termómetro: mide la temperatura interna del dispositivo y la ambiental.
	
\end{itemize}

A los fines prácticos del proyecto para alcanzar la solución propuesta, se utilizó el sensor de GPS. El mismo al ser consumido requiere previamente del permiso del usuario, para su correcto funcionamiento. Tal como fue descripto anteriormente, con dicho sensor es posible obtener la localización del usuario, donde los datos a recibir son la latitud y longitud. Con las presentes coordenadas geográficas, luego se podrá ubicar en el mapa al usuario para poder crear por ejemplo un mapa estadístico de respuestas, realizar un análisis zonal de las mismas, etc.
