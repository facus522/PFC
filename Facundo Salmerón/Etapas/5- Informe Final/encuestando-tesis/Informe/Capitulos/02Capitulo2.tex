...
\begin{figure}[h!]
	\begin{center}
		...
		\small\textsuperscript{\textit{Imagen: https://developer.android.com/guide/components/activities}}
		\label{fig:cicloVida}
	\end{center}
\end{figure}
...
En la esta sección se hará una presentación más amplia sobre cada una de ellas, en relación a lo presentado en la Sección \ref{cap2:subsec:apis}.
...
Como se puede ver, esta API presenta una gran variedad de servicios que facilitan el desarrollo de aplicaciones móviles. Entre las más destacadas se presentan las siguientes \cite{firebase2}:
...
\begin{figure}[h!]
	\makebox[\textwidth][c]{\includegraphics[width=1.5\textwidth, height=1.6\textwidth]{Imagenes/Bitmap/modelo_aplicacion}}%
...
Para agregar preguntas a la encuesta, se debe presionar sobre el botón verde con un \textit{signo de pregunta} (Figura \ref{fig:tiporespuestasl}). El mismo despliega una serie de opciones en relación al tipo de respuesta que tendrá y presionando sobre uno de ellos, se direccionará a la correspondiente alta.
\\
...
En el caso de las preguntas con tipo de respuesta \textit{Escala}, además de ingresar la pregunta en cuestión en el campo editable, posee otro campo que despliega un \textit{NumberPicker}, eligiendo de esta forma la escala máxima a responder, partiendo desde el 1 (Figura \ref{fig:escala} y \ref{fig:valoresescala}).
\\
